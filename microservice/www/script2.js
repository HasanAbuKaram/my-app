...
document.getElementById('request-form').addEventListener('submit', async function(event) {
    ...
    // Get all table data, excluding the "actions" column
    const tableData = table.getData().map(row => {
        const { actions, ...rest } = row; // Destructure and exclude the "actions" column
        return rest; // Return the rest of the row data without "actions"
    });

    // Serialize the filtered table data into a JSON string and append it to FormData
    formData.append("tableData", JSON.stringify(tableData)); // Send the entire filtered table data as JSON

    // Logging FormData entries for debugging purposes
    for (const [key, value] of formData.entries()) {
        console.log(`${key}: ${value}`);
    }

    try {
        const response = await fetch('http://localhost:8081/submit', { // Adjust the URL as needed (http://localhost:8081)
            method: 'POST',
        ...
        if (response.ok) {
            ...
            document.getElementById('request-form').reset(); // Clear the form
            ...
            // Clear the Tabulator table data
            table.clearData();
        } else {
